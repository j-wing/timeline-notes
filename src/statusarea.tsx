...
class SyncStatusArea extends React.Component<
  SyncStatusAreaProps,
  SyncStatusAreaState
> {
  constructor(props: SyncStatusAreaProps) {
    ...
    DriveSyncHandler.addSyncStatusChangeHandler(
    ...
    switch (this.state.syncStatus) {
      ...
      case DriveSyncStatus.SIGNED_OUT:
        ...
        text = (
          <a
            href="#"
            onClick={(e: React.MouseEvent<any>) => this.props.signInHandler()}
          >
            Not signed into Drive
          </a>
        );
        break;
      default:
...
class LockIcon extends React.Component<LockIconProps, LockIconState> {
  render() {
    let visibleText = this.props.noteLocked ? "visible" : "hidden";

    return (
      <span
        className={"note-locked-icon " + visibleText}
        title="Timestamps are locked. Unlock via the menu."
      ></span>
    );
  }
}
  ...
  render() {
    return (
      <div className="status-area">
        <SyncStatusArea
          noteDriveId={this.props.noteDriveId}
