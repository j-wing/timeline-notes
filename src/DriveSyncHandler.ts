...
const CLIENT_ID =
  "370774814885-7kseam56ntmpklgnr9p00f8gh968cmtb.apps.googleusercontent.com";
...
class DriveSyncHandler {
  ...
  initClient() {
    gapi.client
      ...
      .then(() => {
        ...
        gapi.auth2
          .getAuthInstance()
          .isSignedIn.listen(this.handleIsSignedInState.bind(this));

        this.handleIsSignedInState(
  ...
  handleIsSignedInState(isSignedIn: boolean) {
    ...
    let signInState = isSignedIn
      ? DriveSignInState.SIGNED_IN
      : DriveSignInState.SIGNED_OUT;
